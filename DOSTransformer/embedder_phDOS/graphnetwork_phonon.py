...
class Graphnetwork_phonon(nn.Module):
    def __init__(self, layers, n_atom_feats, n_bond_feats, n_hidden, dim_out, device):
        ...
        self.stacked_processor = nn.ModuleList(
            [Processor(EdgeModel(n_hidden), NodeModel(n_hidden)) for i in range(layers)]
        )
        self.GN_decoder = Decoder(n_hidden)
        self.device = device
        self.out_layer = nn.Sequential(
            nn.Linear(n_hidden * 2, n_hidden), nn.LeakyReLU(), nn.Linear(n_hidden, 1)
        )
    ...
    def forward(self, g):
        ...
        x, edge_attr, energies = self.GN_encoder(
            x=g.x, edge_attr=edge_attr, batch=g.batch, energies=energies
        )

        for processor in self.stacked_processor:
            out_x, out_edge_attr = processor(
                x=x, edge_index=g.edge_index, edge_attr=edge_attr
            )
            x = x + out_x
...
class Encoder1(nn.Module):
    def __init__(self, n_atom_feats, n_bond_feats, n_hidden):
        ...
        self.node_encoder_prompt = nn.Sequential(
            nn.Linear(n_atom_feats + n_hidden // 2, n_hidden),
            nn.PReLU(),
