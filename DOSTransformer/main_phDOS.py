...
def main():
    ...
    idx_train, idx_valid, idx_test = train_valid_test_split(
        df, species, valid_size=0.1, test_size=0.1, seed=args.random_state, plot=False
    )

    # load train/valid/test indices
    with open("./data/processed/idx_train.txt", "r") as f:
        idx_train = [int(i.split("\n")[0]) for i in f.readlines()]
    with open("./data/processed/idx_valid.txt", "r") as f:
        idx_valid = [int(i.split("\n")[0]) for i in f.readlines()]
    with open("./data/processed/idx_test.txt", "r") as f:
        idx_test = [int(i.split("\n")[0]) for i in f.readlines()]

    # Format dataloaders
    batch_size = 1
    train_loader = DataLoader(
        df.iloc[idx_train]["data"].values, batch_size=batch_size, shuffle=True
    )
    ...
    if embedder == "dostransformer_phonon":
        ...
        for epoch in range(args.epochs):
            ...
            for bc, batch in enumerate(train_loader):
                ...
                sys.stdout.write(
                    "\r[ epoch {}/{} | batch {}/{} ] Total Loss: {:.4f}".format(
                        epoch + 1, args.epochs, bc + 1, num_batch, loss
                    )
                )
                sys.stdout.flush()
            ...
            if (epoch + 1) % args.eval == 0:
                ...
                print(
                    "\n[ {} epochs ]valid_rmse:{:.4f}|valid_mse:{:.4f}|valid_mae:{:.4f}|valid_r2:{:.4f}".format(
                        epoch + 1, valid_rmse, valid_mse, valid_mae, valid_r2
                    )
                )
                ...
                if valid_rmse < best_rmse and valid_mae < best_mae:
                    ...
                    print(
                        "\n[ {} epochs ]System:test_rmse:{:.4f}|test_mse:{:.4f}|test_mae:{:.4f}|test_r2:{:.4f}".format(
                            epoch + 1, test_rmse, test_mse, test_mae, test_r2
                        )
                    )
                ...
                st_best_sys = "**System [Best epoch: {}] Best RMSE: {:.4f}|Best MSE: {:.4f} |Best MAE: {:.4f}|Best R2: {:.4f}**\n".format(
                    best_epoch, test_rmse, test_mse, test_mae, test_r2
                )
                print(st_best_sys)
