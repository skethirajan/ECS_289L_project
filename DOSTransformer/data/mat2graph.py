...
def load_dataset(
    mp_data,
    dos_data,
    original_dataset_mp_id,
    path_elem_embs="./matscholar-embedding.json",
):
    ...
    for i in tqdm(range(0, len(mp_data))):
        ...
        try:
            if mp_id in original_dataset_mp_id:
                y = torch.tensor(
                    dos_data[mp_id]["densities_total_1"], dtype=torch.float
                )
                y_ft = torch.tensor(
                    dos_data[mp_id]["densities_total_1_ft"], dtype=torch.float
                )
                glob = torch.tensor(
                    [
                        mp_data[mp_id]["energy_per_atom"],
                        mp_data[mp_id]["formation_energy_per_atom"],
                    ],
                    dtype=torch.float,
                )

                cg = get_crystal_graph(
                    elem_feats, str_cif, n_edge_feats=n_bond_feats, radius=8
                )
                cg.y = y / y.max()
                ...
                cg.glob = glob
                cg.mp_id = mp_id
                cg.band_gap = torch.tensor(
                    mp_data[mp_id]["band_gap"], dtype=torch.float
                )
...
def get_crystal_graph(elem_feats, str_cif, n_edge_feats, radius):
    ...
    atom_feats, bonds, bond_feats, atom_coords, atom_attr = get_graph_info(
        crystal, str_cif, elem_feats, n_edge_feats, radius
    )
...
def get_graph_info(crystal, str_cif, elem_feats, n_bond_feats, radius):
    atom_coords, atom_feats, atom_attr, list_nbrs = get_atom_info(
        crystal, elem_feats, radius
    )
    pdist = pairwise_distances(atom_coords)
    bonds, bond_feats = get_bond_info(
        crystal, list_nbrs, str_cif, pdist, n_bond_feats, radius
    )
...
class GaussianDistance(object):
    ...
    def expand(self, distances):
        ...
        return numpy.exp(
            -((distances[..., numpy.newaxis] - self.filter) ** 2) / self.var**2
        )
...
def get_atom_info(crystal, elem_feats, radius):
    ...
    atom_feats = numpy.vstack(
        [elem_feats[crystal[i].specie.number - 1] for i in range(len(crystal))]
    )

    atom_attr = numpy.vstack(
        [type_onehot[crystal[i].specie.number - 1] for i in range(len(crystal))]
    )
...
def get_bond_info(crystal, list_nbrs, str_cif, pdist, n_bond_feats, radius):
    ...
    index1 = (
        torch.LongTensor([i for i in range(len(crystal))])
        .reshape(-1, 1)
        .expand(nbr_fea_idx.shape)
        .reshape(1, -1)
    )
