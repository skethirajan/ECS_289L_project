...
def parse_args():
    ...
    parser.add_argument(
        "--layers", "-l", type=int, default=3, help="The number layers of the Processor"
    )
    parser.add_argument(
        "--transformer",
        "-t",
    ...
    parser.add_argument(
        "--embedder", type=str, default="DOSTransformer", help="Embedder"
    )
    parser.add_argument("--hidden", type=int, default=256, help="Hidden dim")
    parser.add_argument(
        "--random_state", type=int, default=0, help="Random state for dataset split"
    )
    parser.add_argument(
        "--dataset",
        type=str,
        default="whole",
        help="Dataset: ood_crystal or ood_element or whole",
    )
    parser.add_argument(
        "--attn_drop", type=float, default=0.0, help="attention dropout ratio"
    )
    parser.add_argument("--seed", type=int, default=0, help="Random seed")
    parser.add_argument(
        "--beta", type=float, default=1.0, help="alpha for the spark loss2"
    )
...
def test(model, data_loader, criterion, r2, device):
    ...
    with torch.no_grad():
        ...
        for bc, batch in enumerate(data_loader):
            ...
            embeddings = scatter_sum(
                embeddings, batch.batch, dim=0
            )  # For dos_system embeddings
...
def load_data(filename):
    ...
    try:
        ...
        df["structure"] = (
            df["structure"].apply(eval).progress_map(lambda x: Atoms.fromdict(x))
        )
        ...
        df["species"] = df["structure"].map(
            lambda x: list(set(x.get_chemical_symbols()))
        )
...
def build_data(entry, r_max=5.0):
    ...
    if entry.crystal_system == "cubic":
        ...
        system = 5
    else:  # no crystal_system found with Pymatgen
        system = 6
    data = Data(
        pos=positions,
        lattice=lattice,
